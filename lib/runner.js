...
/**
 * cpp 파일명에서 과제 번호를 감지한다.
 * @param {string} cppPath
 * @returns {string|null}
 */
function detectHwNum(cppPath) {
  const base = path.basename(cppPath).toLowerCase();
  const m = base.match(/hw(\d+)/) || base.match(/lab(\d+)/);
  return m ? m[1] : null;
}

module.exports = { detectSubProblem, detectHwNum, runTests };
