const { execSync } = require('child_process');
const path = require('path');
const fs = require('fs');
const os = require('os');

// void main() 또는 void main(void) → int main() / int main(void)
const VOID_MAIN = /\bvoid(\s+main\s*\()/g;

// macOS/Linux g++에서 Windows 전용 _s 함수들을 표준 함수로 매핑하는 호환 매크로
const COMPAT_MACROS = `#define _CRT_SECURE_NO_WARNINGS
// _s 함수 → 표준 함수 호환 매크로
#ifndef _WIN32
#define scanf_s scanf
#define sscanf_s sscanf
#define fscanf_s fscanf
#define printf_s printf
#define fprintf_s fprintf
#define gets_s(buf, ...) fgets(buf, 4096, stdin)
#define fgets_s(buf, size, stream) fgets(buf, size, stream)
#define sprintf_s(buf, size, ...) sprintf(buf, __VA_ARGS__)
#define snprintf_s(buf, size, ...) snprintf(buf, size, __VA_ARGS__)
#define strcat_s(dst, size, src) strcat(dst, src)
#define strcpy_s(dst, size, src) strcpy(dst, src)
#define strtok_s(str, delim, ctx) strtok(str, delim)
#define fopen_s(fp, name, mode) (*(fp) = fopen(name, mode), *(fp) ? 0 : 1)
#define memcpy_s(dst, dsz, src, cnt) memcpy(dst, src, cnt)
#define memmove_s(dst, dsz, src, cnt) memmove(dst, src, cnt)
#endif
`;

/**
 * void main → int main 변환 후 main 함수 끝 } 직전에 return 0; 을 삽입한다.
 * 이미 return 문이 있으면 삽입하지 않는다.
 * 브레이스 카운터를 사용해 main의 정확한 닫는 }를 찾는다.
 */
function fixVoidMain(src) {
  if (!VOID_MAIN.test(src)) return src;
  VOID_MAIN.lastIndex = 0;

  let result = src.replace(VOID_MAIN, 'int$1');

  // main 함수의 여는 { 위치 찾기
  const mainOpenMatch = result.match(/\bint\s+main\s*\([^)]*\)\s*\{/);
  if (!mainOpenMatch) return result;

  const mainBodyStart = mainOpenMatch.index + mainOpenMatch[0].length;

  // 브레이스 카운터로 main의 정확한 닫는 } 위치 탐색
  let depth = 1;
  let i = mainBodyStart;
  while (i < result.length && depth > 0) {
    if (result[i] === '{') depth++;
    else if (result[i] === '}') depth--;
    i++;
  }
  if (depth !== 0) return result; // 브레이스 불일치

  const mainClosePos = i - 1;
  let mainBody = result.substring(mainBodyStart, mainClosePos);

  // main body 안의 bare return; → return 0; (void 함수는 건드리지 않음)
  mainBody = mainBody.replace(/\breturn\s*;/g, 'return 0;');

  // main에 return이 없으면 닫기 전에 return 0; 삽입
  if (!/\breturn\b/.test(mainBody)) {
    mainBody += '\n    return 0;\n';
  }

  return result.substring(0, mainBodyStart) + mainBody + result.substring(mainClosePos);
}

/**
 * Windows _s 함수 호환 매크로를 주입한 임시 파일을 생성한다.
 * 원본을 수정하지 않으며, 임시 파일 경로를 반환한다.
 */
function patchSource(cppPath) {
  const raw = fs.readFileSync(cppPath, 'utf8');
  const original = raw.replace(/^\uFEFF/, '');
  const patched = COMPAT_MACROS + fixVoidMain(original);
  const ext = path.extname(cppPath);
  const base = path.basename(cppPath, ext);
  const tmpPath = path.join(os.tmpdir(), `${base}__patched_${process.pid}${ext}`);
  fs.writeFileSync(tmpPath, patched, 'utf8');
  return tmpPath;
}

/**
 * .cpp 파일을 g++로 컴파일하고 결과를 반환한다.
 * Windows _s 함수는 호환 매크로로 자동 처리한다.
 * @param {string} cppPath - 컴파일할 .cpp 파일 경로
 * @returns {{ success: boolean, score: number, error: string|null, binaryPath: string|null }}
 */
function compile(cppPath) {
  const ext = path.extname(cppPath).toLowerCase();
  const base = path.basename(cppPath, ext);
  const binaryPath = path.join(os.tmpdir(), `${base}__bin_${process.pid}`);
  const compiler = ext === '.c' ? 'gcc' : 'g++';
  const tmpPath = patchSource(cppPath);

  try {
    execSync(`${compiler} -o "${binaryPath}" "${tmpPath}" 2>&1`, {
      timeout: 30000,
      encoding: 'utf8',
    });
    return { success: true, score: 0, error: null, binaryPath };
  } catch (err) {
    const errorOutput = err.stdout || err.stderr || err.message || '알 수 없는 컴파일 오류';
    if (fs.existsSync(binaryPath)) fs.unlinkSync(binaryPath);
    // 입출력 구문 존재 시 부분 점수 3점 (criteria.md: 컴파일 실패 + 입출력 구문 존재 → 3점)
    let score = 0;
    try {
      const src = fs.readFileSync(cppPath, 'utf8');
      if (/scanf|cin|printf|cout|gets\b|fgets/.test(src)) score = 3;
    } catch (_) {}
    return { success: false, score, error: errorOutput.trim(), binaryPath: null };
  } finally {
    if (fs.existsSync(tmpPath)) fs.unlinkSync(tmpPath);
  }
}

/**
 * 컴파일된 바이너리를 제거한다.
 * @param {string} binaryPath
 */
function cleanup(binaryPath) {
  if (binaryPath && fs.existsSync(binaryPath)) {
    fs.unlinkSync(binaryPath);
  }
}

module.exports = { compile, cleanup };
