 ...
 * Gemini CLI가 이 출력을 읽어 criteria.md를 직접 작성한다.
 *
...
function main() {
  ...
  // criteria.md 가 PPTX 보다 최신이면 재추출 생략 (--force 로 강제 실행 가능)
  ...
  if (!force && fs.existsSync(criteriaPath)) {
    ...
    const pptxMtime = fs.statSync(pptxPath).mtime;
    if (criteriaMtime > pptxMtime) {
      console.error(`[스킵] criteria.md 가 PPTX(${path.basename(pptxPath)})보다 최신입니다. 재추출 생략.`);
  ...
  // stdout으로 추출된 텍스트 출력 — Gemini CLI가 읽어서 criteria.md 작성
