...
const { detectSubProblem, detectHwNum, runTests } = require('./runner');
...
const defaultOutputDir = path.resolve('output', 'batches');
fs.mkdirSync(defaultOutputDir, { recursive: true });
const outputPath = outputArg !== -1 ? process.argv[outputArg + 1] : path.join(defaultOutputDir, `prepare_batch_${Date.now()}.json`);
...
for (const zipFilename of zipFiles) {
  ...
  const hwNum = cppFiles.map(detectHwNum).find(Boolean) || '?';

  if (cppFiles.length === 0) {
    results.push({
      hwNum, studentId: extracted.studentId,
  ...
  for (const cppPath of cppFiles) {
    ...
    // skipReview=true 항목의 감점 문자열을 사전 계산 (Claude가 누락하는 것을 방지)
    const deductions = autoDeductions.length > 0
      ? autoDeductions.map(d => `-${Math.abs(d.points)}: ${d.label}`).join('\n')
      : '';
